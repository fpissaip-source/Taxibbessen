  ...
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star key={i} className={`w-3.5 h-3.5 ${i < rating ? "fill-primary text-primary" : "fill-muted/40 text-muted/40"}`} />
      ))}
    </div>
  );
}
...
export function ReviewCarousel() {
  ...
  return (
    ...
    >
      ...
      <div className="flex w-max rv-hide py-2">
        {duplicated.map((review, i) => (
          <div key={i} className="w-[320px] sm:w-[380px] mx-3 bg-card border border-border rounded-2xl p-5 sm:p-6 shadow-md flex-shrink-0">
            <div className="flex items-start justify-between gap-2 mb-3">
              ...
              <span className="flex items-center gap-1 text-[10px] font-semibold bg-white/8 text-white/60 px-2 py-0.5 rounded-full whitespace-nowrap border border-white/10">
                <svg viewBox="0 0 24 24" className="w-2.5 h-2.5 shrink-0" xmlns="http://www.w3.org/2000/svg">
